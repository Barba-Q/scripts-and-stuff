...
# ==============================================================================
# Script to prepare NVIDIA driver installation with DKMS (v1.1)
# for the 'nvidia-open' kernel modules on Void Linux.
#
# Version: 1.1 (With optional installer start at the end)
#
...
if [ -z "$1" ]; then
    ...
    echo -e "Example: sudo ./setup_nvidia_dkms.sh ./NVIDIA-Linux-x86_64-550.78.run"
    exit 1
fi
...
xbps-install -Syu
xbps-install -Sy base-devel linux-headers dkms
...
TEMP_EXTRACT_DIR="/tmp/nvidia-installer-extraction"
...
EXTRACTED_DIR=$(find "$TEMP_EXTRACT_DIR/extract" -type d -name "NVIDIA-Linux-x86_64-*")
cd "$EXTRACTED_DIR"
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    # Execute the installer and instruct it to skip the kernel part
